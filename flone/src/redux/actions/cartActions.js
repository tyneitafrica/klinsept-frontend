...
export const addToCart = (item, addToast, quantityCount=1) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post("https://klinsept-backend.onrender.com/api/v1.0/cart/add/", {
        product_id: item.id,
        quantity: quantityCount,
      },
      {
...
export const deleteFromCart = (itemId, addToast) => {
  return async (dispatch) => {
    try {
      const response = await axios.delete(`https://klinsept-backend.onrender.com/api/v1.0/cart/remove${itemId}`,
       {
          headers: {
            "x-api-key": "f6c52669-b6a9-4901-8558-5bc72b7e983a", // Replace with your actual API key
            "Content-Type": "application/json",
      ...
      if (addToast) {
        addToast("Failed to Remove Item from Cart", { appearance: "error", autoDismiss: true });
      }
